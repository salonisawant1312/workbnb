...
const getHostBookings = async (req, res) => {
  try {
    const bookings = await Booking.find({ hostId: req.user._id })
      .populate('listingId', 'title address images workspaceType')
...
const cancelBooking = async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.id);
    if (!booking) return res.status(404).json({ success: false, message: 'Booking not found' });
    if (booking.guestId.toString() !== req.user._id.toString() && booking.hostId.toString() !== req.user._id.toString() && req.user.role !== 'admin') {
      return res.status(403).json({ success: false, message: 'Not allowed' });
    }
...
module.exports = { createBooking, getUserBookings, getHostBookings, getBooking, updateBooking, cancelBooking, confirmBooking };
