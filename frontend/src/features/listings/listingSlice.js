...
export const fetchHostListings = createAsyncThunk('listings/fetchHost', async (_, thunkAPI) => {
  try {
    const { data } = await api.get('/listings/my-listings');
    return data.data;
  } catch (err) {
    return thunkAPI.rejectWithValue(err.response?.data?.message || 'Failed to fetch your listings');
  }
});
...
const listingSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchHostListings.pending, (state) => { state.loading = true; state.error = null; })
      .addCase(fetchHostListings.fulfilled, (state, action) => { state.loading = false; state.items = action.payload; })
      .addCase(fetchHostListings.rejected, (state, action) => { state.loading = false; state.error = action.payload; })
