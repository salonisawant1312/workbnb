  ...
  return (
    <section className="space-y-5">
      ...
      <div className="mt-1 flex flex-wrap items-center gap-3">
        <p className="text-slate-600">★ {selected.rating || 4.8} • {selected.address?.city || 'Prime district'}</p>
        {(() => {
          const s = selected.availabilityStatus || 'available';
          const map = {
            available:   { label: 'Available Now',  cls: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500' },
            unavailable: { label: 'Unavailable',    cls: 'bg-slate-100 text-slate-500 border-slate-200',   dot: 'bg-slate-400' },
            booked:      { label: 'Currently Booked', cls: 'bg-red-50 text-red-600 border-red-200',        dot: 'bg-red-500' },
          };
          const cfg = map[s] || map.available;
          return (
